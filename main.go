...
func home(w http.ResponseWriter, r *http.Request) {
	fmt.Fprintf(w, "<html>this site scrapes the doujinstyle discord<br><a href='/comiket'>comiket</a>, <a href='/reitaisai'>reitaisai</a>, <a href='/m3'>m3</a><br>this server doesn't host any content")
	return
}
...
func handler(db *bolt.DB) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Println(data)
		if data == nil {
			w.Write([]byte("no data yet, check back later"))
			return
		}
...
func main() {
	var tk = flag.String("token", "NULL", "Discord API token (user)")
	flag.Parse()
	if *tk == "NULL" {
